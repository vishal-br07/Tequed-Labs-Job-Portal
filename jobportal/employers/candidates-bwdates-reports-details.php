...
<title>Tequed Labs Jobportal-Candidates Profiles</title>
...
?>
<h2 align="center" style="color:purple">Report from <?php echo $fdate?> to <?php echo $tdate?></h2>
<hr />
<?php
$eid=$_SESSION['emplogin'];
  ...
  $next_page = $page_no + 1;
  $adjacents = "2"; 
$ret = "SELECT tbljobseekers.*,tbljobs.*,tblapplyjob.Status, tblapplyjob.UserId, tblapplyjob.JobId,tblapplyjob.Applydate, tblapplyjob.ResponseDate from tblapplyjob join tbljobs on tblapplyjob.JobId=tbljobs.jobId join tbljobseekers  on tblapplyjob.UserId=tbljobseekers.id where tbljobs.employerId=:eid && date(tblapplyjob.Applydate) between '$fdate' and '$tdate'";
     ...
     ;?></a>  </div>
                  ...
                  <div class="btn-row"> <a href="../Jobseekersresumes/<?php echo htmlentities($row->Resume);?>" class="resume" target="_blank"><i class="fa fa-file-text-o"></i>Resume</a> 
                    <a href="candidates-details.php?canid=<?php echo ($row->id);?>" class="contact" target="_blank">View Candidate Detail</a> 
                       <a href="app-details.php?jobid=<?php echo ($row->JobId);?> && name=<?php echo htmlentities ($row->FullName);?>&& jsid=<?php echo htmlentities ($row->id);?>" class="login" target="_blank">Application Details</a>
                  </div>
...
<h4 align="center" style="color:purple">No record found</h4>
